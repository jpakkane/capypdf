...
int main(int argc, char **argv) {
    ...
    try {
        PdfGen gen("test.pdf", opts);
        {
            ...
            ctx.cmd_S();
            ctx.simple_ascii_text(
                "This is text in Times New Roman.", FONT_TIMES_ROMAN, 12, 100, 500);
            ctx.simple_ascii_text("This is text in Helvetica.", FONT_HELVETICA, 12, 100, 480);
            ctx.simple_ascii_text("This is text in Courier.", FONT_COURIER, 12, 100, 460);
            ...
            ctx.simple_ascii_text("GOLD!", FONT_HELVETICA_BOLD, 32, 250, 340);
        }
        {
            auto ctx = gen.new_page();
