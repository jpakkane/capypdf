...
public:
    ...
    CapyPDF_IccColorSpaceId add_icc_profile(const char *bytes, uint64_t bufsize, int32_t num_channels) {
        CapyPDF_IccColorSpaceId cpid;
        CAPY_CPP_CHECK(
            capy_generator_add_icc_profile(*this, bytes, bufsize, num_channels, &cpid));
        return cpid;
    }
