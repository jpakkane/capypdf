...
PdfDocument::add_page_labeling(uint32_t start_page,
    ...
    page_labels.emplace_back(start_page, style, prefix, start_num);
  ...
  /Pages {} 0 R
    ...
    << /Nums [
        ...
        for(const auto &page_label : page_labels) {
            ...
            if(page_label.style) {
                std::format_to(app, "/S {} ", page_label_types.at(*page_label.style));
            }
            if(page_label.prefix) {
                std::format_to(app, "/P {} ", utf8_to_pdfutf16be(*page_label.prefix).c_str());
            }
            if(page_label.start_num) {
                std::format_to(app, "/St {} ", *page_label.start_num);
            }
            buf += R"(>>
)";
        }
