...
CAPYPDF_PUBLIC CapyPDF_EC capy_document_properties_set_page_mode(
    CapyPDF_DocumentProperties *docprops, CapyPDF_Page_Mode mode) CAPYPDF_NOEXCEPT {
    API_BOUNDARY_START;
    auto *dp = static_cast<DocumentProperties *>(docprops);
    dp->page_mode = mode;
    RETNOERR;
...
CAPYPDF_PUBLIC CapyPDF_EC capy_form_xobject_new(CapyPDF_Generator *gen,
    ...
    *out_ptr = static_cast<CapyPDF_DrawContext *>(g->new_form_xobject(bbox.value()));
    RETNOERR;
    API_BOUNDARY_END;
}
...
CAPYPDF_PUBLIC CapyPDF_EC capy_transparency_group_new(
    ...
    *out_ptr = static_cast<CapyPDF_DrawContext *>(g->new_transparency_group(bbox.value()));
    RETNOERR;
    API_BOUNDARY_END;
}
...
CAPYPDF_PUBLIC CapyPDF_EC capy_tiling_pattern_context_new(CapyPDF_Generator *gen,
    ...
    *out_ptr = static_cast<CapyPDF_DrawContext *>(g->new_color_pattern(bbox.value()));
    RETNOERR;
    API_BOUNDARY_END;
}
...
CAPYPDF_PUBLIC CapyPDF_EC capy_graphics_state_set_blackpoint_compensation(
    CapyPDF_GraphicsState *state, CapyPDF_Black_Point_Compensation use_comp) CAPYPDF_NOEXCEPT {
    API_BOUNDARY_START;
    auto *s = static_cast<GraphicsState *>(state);
...
CAPYPDF_PUBLIC CapyPDF_EC capy_raster_image_builder_new(CapyPDF_RasterImageBuilder **out_ptr)
    CAPYPDF_NOEXCEPT {
    API_BOUNDARY_START;
    *out_ptr = new RasterImageBuilder;
    RETNOERR;
    API_BOUNDARY_END;
}
...
CAPYPDF_PUBLIC CapyPDF_EC capy_raster_image_builder_destroy(CapyPDF_RasterImageBuilder *builder)
    CAPYPDF_NOEXCEPT {
    API_BOUNDARY_START;
    auto *ri = static_cast<RasterImageBuilder *>(builder);
    delete ri;
    RETNOERR;
    API_BOUNDARY_END;
}
...
CAPYPDF_PUBLIC CapyPDF_EC capy_square_annotation_new(CapyPDF_Annotation **out_ptr)
    CAPYPDF_NOEXCEPT {
    API_BOUNDARY_START;
    *out_ptr = new Annotation{{}, SquareAnnotation{}, {}, {}};
    RETNOERR;
    API_BOUNDARY_END;
}
...
CAPYPDF_PUBLIC CapyPDF_EC capy_annotation_set_RD(
    CapyPDF_Annotation *annotation, double x0, double y0, double x1, double y1) CAPYPDF_NOEXCEPT {
    API_BOUNDARY_START;
    auto *a = static_cast<Annotation *>(annotation);
    if(auto *sq = std::get_if<SquareAnnotation>(&a->sub)) {
        sq->RD = PdfRectangle{x0, y0, x1, y1};
    } else {
        return conv_err(ErrorCode::IncorrectAnnotationType);
    }
    RETNOERR;
