...
public:
  MMapperPrivate(HANDLE fh, HANDLE mh, const void *buf, uint64_t bufsize) : file_handle{fh}, mapping_handle{mh}, buf{buf}, bufsize{bufsize} {}
    ~MMapperPrivate() {
      UnmapViewOfFile(buf);
...
rvoe<MMapper> mmap_file(const char *fname) {
  HANDLE file_handle = CreateFile(fname, GENERIC_READ, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
  ...
  HANDLE mapping_handle = CreateFileMapping(file_handle, nullptr, PAGE_EXECUTE_READ, 0, 0, nullptr);
  if(!mapping_handle) {
    CloseHandle(file_handle);
    RETERR(MMapFail);
  }
